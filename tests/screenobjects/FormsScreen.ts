...
class FormsScreen extends AppScreen {
    ...
    async fillSubmitForm({ input, dropdown }:{input:string; dropdown:string;}) {
        await this.input.setValue(input);
        await this.dropDown.click();
        const option = await $(`android=new UiSelector().textMatches("(?i)${dropdown}")`);
        await option.click();
    }

    async fillInputText({ input }:{input:string;}) {
        await this.input.setValue(input);
    }

    async validatePreview() {
        const inputValue = await this.input.getText();
        const resultValue = await this.inputTextResult.getText();
        
        await expect(resultValue).toBe(inputValue);
    }

    async fillFormWithJson({ textInput, switchVal, dropdown }:{textInput:string; switchVal:boolean; dropdown:string;}) {
        await this.input.setValue(textInput);
        await this.switch.setValue(Number(switchVal));
        await this.dropDown.click();
        const option = await $(`android=new UiSelector().textMatches("(?i)${dropdown}")`);
        await option.click(); 
    }
