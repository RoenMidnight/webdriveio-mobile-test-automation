...
class LoginScreen extends AppScreen {
    ...
    async displayErrorMessage(message:string) {         
        return $(`android=new UiSelector().textMatches("(?i)${message}")`).isDisplayed();
    }
    ...
    async submitSignUpForm({ username, password, confirmation }:{username:string; password:string; confirmation:string;}) {
        await this.email.setValue(username);
        await this.password.setValue(password);
        await this.repeatPassword.setValue(confirmation);
